$(document).ready(function () {
    ...
    function validateForm() {
        ...
        var validator = $("#formMCC").validate({
            rules: {
                MCC_CODE: "required",
                MCC_DESC: "required",
    ...
    $(document).on('click', '#btnAddMsc', function () {
        $('#notyMsc').hide();
        var menuId = $('#menuId').val();
        var urlP = url + 'AddMsc?m=' + menuId;
    ...
    $(document).on('click', '#tableMsc a#delete', function (e) {
        e.preventDefault();
        if (confirm('Are you sure you want to delete record?')) {
            ...
            var editLink = $(this).attr('data-key');
            // alert(editLink);
            var urlP = url + 'DeleteMsc/' + editLink + '?m=' + encodeURIComponent(menuId);
            //var acq = $('#CBN_CODE').val();
    ...
    $(document).on('click', '#tableMsc a#undo', function (e) {
        e.preventDefault();
        //if (confirm('Are you sure you want to delete record?')) {
            //$('#notyMsc').hide();
            var menuId = $('#m').val();
    ...
    function validateMSCForm() {
        ...
        var validator = $("#formMSC").validate({
            rules: {
                CARDSCHEME: "required",
                CHANNEL: "required",
                DOM_MSCVALUE: {required:true, number: true },
                DOMCAP:{ number:true},
                DOM_SETTLEMENT_CURRENCY: "required",
                INT_MSCVALUE: { required: true, number: true },
                INTLCAP: { number: true },
                INT_SETTLEMENT_CURRENCY: "required"
            },
            messages: {
            ...
            submitHandler: function () {
                ...
                //return;
                loaderSpin(true);
                disableButton2(true, 'btnAddMscGrid');
                var ajax = new Ajax();
                ajax.send($reqLogin).done(function (response) {
                    loaderSpin(false);
                    disableButton2(false, 'btnAddMscGrid');
                    if (response.RespCode === 0) {
                       
                        $('#myModalMsc').modal('hide');
                        $('#divAcqMsc').html(response.data_msc);
